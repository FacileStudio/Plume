...
// UserStore resolves a login to a row in Plume's own users table.
//
// It is porte's escape hatch, taken deliberately. porte/pg ships a porte_users
// table and Plume could have moved onto it, but documents, signers, clients,
// spaces and SMTP settings all hang off users(id). Moving it would be a
// rewrite of everything except authentication.
type UserStore struct {
	orm *gorm.DB
}

// NewUserStore creates a UserStore backed by the given gorm.DB.
...
// claim asserts nothing: a provider that stops sending a name should not
// blank it here. oidc_picture_url is written unconditionally because this
// app derives the whole avatar from it.
...
// create is the one place a Plume account comes into existence, whichever
