...
// AdoptPorte moves Plume's credentials onto porte's tables.
//
...
// foreign keys repointed at Plume's own users.
//
...
// Plume has no API token table to fold in: its only credential was the
// session, so there is nothing here but sessions, passwords and the subject.
...
//
// The OIDC tokens deliberately do **not** come across. Plume stores them
